...
enum class BodyPositions {
    ...
    private fun handRaised(handDescriptor: Int, pose: Pose): Boolean {
        ...
        return when (handDescriptor) {
            LEFT_HAND -> isArmRaised(leftElbow, leftWrist) && isArmLowered(rightElbow, rightWrist)
            RIGHT_HAND -> isArmRaised(rightElbow, rightWrist) && isArmLowered(leftElbow, leftWrist)
            BOTH_HANDS -> isArmRaised(leftElbow, leftWrist) && isArmRaised(rightElbow, rightWrist)
